...
- **Driver Assignment System**: Admins assign drivers to routes with vehicles and specify start/end times for each route. Each assignment defines which driver operates which route, with which vehicle, and at what times (e.g., "07:00 - 08:30"). Optional notes field for additional context. Admin view groups assignments by driver with collapsible sections showing route type tags and time ranges. Driver reassignment is done through the edit dialog.
  - **Cascade Updates**: When a driver assignment is updated, all future shifts (date >= today) that were created from that assignment are automatically updated with the new driver, route, vehicle, and times. This ensures data consistency across the application - admins can change assignments in one place and have those changes automatically reflected in the schedule. Frontend cache invalidation ensures the UI updates immediately across all pages.
...
- **Student Attendance Tracking**: Daily attendance tracking system with riding/absent status. Students are assumed to be riding by default until marked absent. The system supports both single-day and date range marking:
  - **Simplified UI**: When no attendance record exists, displays "Riding (default)" text with a single "Mark as Absent" button. When an attendance record exists, shows a status badge and toggle button to switch between riding/absent
  - **Single-Day Marking**: Toggle between riding/absent for today with immediate feedback
  - **Date Range Marking**: When clicking "Mark as Absent", a dialog allows selecting an end date to mark multiple consecutive days at once, solving the multi-day absence problem
...
- **Route Progress Tracking System**: Real-time tracking of driver route progress with parent visibility. The `route_progress` table tracks stop completion status (PENDING/COMPLETED/SKIPPED) for each shift. Drivers can mark stops as completed or skipped during active routes via the routes page. Parents see real-time bus location showing completed stops, current progress, and stops-away-from-pickup calculations. Progress data auto-refreshes every 30 seconds. Backend includes progress initialization, stop status updates, and student-specific progress queries with pickup stop calculations.
- **GPS Infrastructure**: Optional latitude/longitude fields added to the `stops` table for future vehicle GPS device integration. Admin UI includes GPS coordinate input fields in stop creation/editing dialogs with helper text indicating these are reserved for future tracking capabilities. Currently unpopulated but ready for when GPS hardware is installed on vehicles.
- **Lead Driver Permission System**: `isLeadDriver` boolean flag on user accounts enables enhanced permissions for senior drivers. Lead drivers can perform additional route management tasks and access extended features. Managed through admin user interface as a simple checkbox rather than a separate role, allowing flexible permission assignment on existing driver accounts.
