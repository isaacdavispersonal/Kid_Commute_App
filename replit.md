...
- **Type System Enhancements**: `RouteStopWithMetadata` type enriches Stop data with route-stop junction metadata (stopId, routeStopId, stopOrder, scheduledTime), enabling proper frontend access to route ordering information and preventing duplicate stop assignments in route management. Frontend route stop operations (reordering, removal) correctly use `routeStopId` (junction table ID) rather than `stopId` (Stop entity ID) for proper junction table updates.
...
- **Key Features**:
    ...
    - **Driver Assignment System**: Admins assign drivers to routes with optional vehicle assignment. Driver assignments table includes `vehicleId` field which is used when creating shifts from assignments. Shift times are specified during individual shift creation rather than as defaults in assignments.
    - **Parent Student Management**: Parents can edit children's information.
    - **Multi-Route Student Assignment**: Students can be assigned to multiple routes (morning, afternoon, etc.) via studentRoutes junction table. Features include: many-to-many student-route relationships without requiring pickup/dropoff information during initial assignment, unique constraint preventing duplicate assignments (studentId + routeId), validation ensuring student and route exist before assignment, dedicated API endpoints (POST /api/admin/students/:id/routes, DELETE /api/admin/student-routes/:assignmentId, GET /api/admin/students/:id/routes) with proper error handling, admin dialog interface for managing multiple route assignments with add/remove controls, and query invalidation for real-time UI updates. Visual route type indicators display color-coded badges (AM/PM/Extra) with icons (Sunrise/Sunset/Clock) in student cards and assignment dialogs for clear morning/afternoon distinction. Legacy single-route fields (assignedRouteId, pickupStopId, dropoffStopId) remain in students table for backward compatibility but multi-route system is now the primary assignment mechanism. Parent API endpoints (`/api/parent/students` and `/api/parent/vehicle-location`) support both junction table and legacy assignments via effective route selection logic - prioritizes studentRoutes entries, falls back to legacy assignedRouteId for backward compatibility.
    ...
    - **Parent Dashboard Enhancements**: Live ETA countdown, quick contact actions, driver information display, and account management (including deletion).
    - **Payment Portal Configuration**: Environment-driven billing portal system for parent payments. Payment portals (QuickBooks, ClassWallet) are configured via environment variables (QUICKBOOKS_PORTAL_URL, CLASSWALLET_PORTAL_URL) and automatically seeded into the `payment_portals` table on server startup. Backend API (GET /api/billing/portals) returns only enabled portals. Parent dashboard conditionally renders billing card only when portals are configured, displaying dynamic buttons with tenant-specific portal URLs. Supports future multi-tenant configurations and authenticated session link generation.
    ...
    - **Live Fleet Map**: Admin page displaying real-time vehicle locations with auto-refresh every 15 seconds. Map preserves user's zoom level and pan position across refreshes using `hasInitializedBoundsRef` to only fit bounds on initial load. Includes fleet metrics and vehicle status indicators.
    ...
    - **Route Groups System**: Organizational system for managing multiple routes under shared groups. Route Groups allow organizing routes by name/description for fleet management via route_groups table and groupId foreign key. Admin UI provides integrated group management directly within the Routes tab using collapsible group sections, with inline controls for creating, editing, and deleting groups. Routes are automatically displayed grouped by their assigned route group, with ungrouped routes shown separately. Each group section displays name, description, route count, and inline edit/delete buttons.
