...
- **Key Features**:
    ...
    - **Driver Assignment System**: Admins assign drivers to routes with optional vehicle assignment and default start/end times. Driver assignments table includes `vehicleId`, `startTime`, and `endTime` fields which serve as defaults when creating shifts from assignments. These values can be overridden during individual shift creation.
