...
- **Driver Assignment System**: Admins assign drivers to routes with vehicles for recurring schedules. Assignments are general templates without specific dates/times - they define which driver operates which route with which vehicle. Optional schedule patterns (weekdays, daily, specific days) and notes can be added. Admin view groups assignments by driver with collapsible sections showing route type tags. Driver reassignment is done through the edit dialog.
