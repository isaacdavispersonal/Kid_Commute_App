...
- **Assignment-Based Scheduling**: Simplified schedule creation where shifts are generated from existing driver assignments. Two scheduling methods:
  ...
  - **Bulk Scheduling**: Admins select drivers, date range, and days of week. The system automatically creates shifts for all selected drivers' assignments on matching dates. All shift details (route, vehicle, times, shift type) are inherited from the driver assignments, eliminating manual data entry and ensuring consistency.
